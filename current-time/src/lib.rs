...
mod tests {
    ...
    fn test_adjust_reason_multiple_flags() {
        ...
        let decoded = CurrentTime::from_bytes(bytes);
        assert!(decoded.adjust_reason.contains(AdjustReason::MANUAL_TIME_UPDATE));
        assert!(decoded.adjust_reason.contains(AdjustReason::DST_CHANGE));
        assert!(decoded.adjust_reason.contains(AdjustReason::EXTERNAL_REFERENCE));
        assert!(!decoded.adjust_reason.contains(AdjustReason::TIMEZONE_CHANGE));
    }
